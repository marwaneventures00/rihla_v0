    ...
    <div className="min-h-screen bg-[#050508] text-white overflow-hidden">
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) {
          .mesh-bg, .orb, .shimmer::after, .cta-gradient { animation: none !important; }
        }
      `}</style>

      <section className="relative min-h-screen flex items-center justify-center px-6 py-20 mesh-bg">
        ...
        <div className="orb orb3 w-[300px] h-[300px] bg-[#C8102E]/15 top-[30%] right-[20%]" />
        ...
        <div className="relative z-10 max-w-5xl text-center">
          <motion.div
            ...
            className="inline-flex shimmer items-center rounded-full border border-[#2A2A4A] px-4 py-2 text-xs sm:text-sm text-[#C9C9E8] bg-gradient-to-r from-[#17172A] to-[#101021]"
          >
          ...
          <motion.p
            ...
            className="mt-8 mx-auto max-w-3xl text-lg sm:text-xl text-[#A0A0B8]"
          >
          ...
          >
            ...
            <Link
              to="/auth"
              className="rounded-xl px-7 py-3.5 text-sm sm:text-base font-medium border border-white/35 text-white hover:bg-white hover:text-[#0A0A14] transition-all"
            >
              For universities
            </Link>
          </motion.div>

          <p className="mt-6 text-xs sm:text-sm text-[#666680]">
            Trusted by leading Moroccan universities · Powered by frontier AI
          </p>
        </div>
      </section>

      <AnimatedSection className="border-y border-[#1E1E35] bg-[#0A0A14] px-6 py-12">
        <div className="max-w-6xl mx-auto grid grid-cols-2 lg:grid-cols-4 gap-8">
          {STATS.map((s) => (
            <div key={s.label} className="text-center lg:text-left">
              ...
              <p className="mt-2 text-sm text-[#A0A0B8]">{s.label}</p>
            </div>
          ))}
        </div>
      </AnimatedSection>
      ...
      <AnimatedSection className="px-6 py-24">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-center text-[#A0A0B8] mt-4">Three AI-powered modules working together</p>
          <div className="mt-12 grid md:grid-cols-3 gap-5">
            {FEATURES.map((f) => (
              <motion.article
                whileHover={reduceMotion ? {} : { scale: 1.02 }}
                key={f.title}
                className="group rounded-2xl bg-[#0F0F1A] border border-[#1E1E35] p-6 hover:shadow-[0_0_36px_rgba(99,102,241,0.22)] transition-all"
              >
                <div className={`inline-flex p-3 rounded-xl bg-gradient-to-r ${f.gradient}`}>
                  <f.icon className="w-5 h-5" />
                </div>
                <h3 className="mt-5 text-xl font-semibold">{f.title}</h3>
                <p className="mt-3 text-[#A0A0B8] leading-relaxed">{f.body}</p>
                <p className="mt-6 text-xs text-[#C9C9E8]">{f.tag}</p>
              </motion.article>
            ))}
          </div>
        </div>
      ...
      <AnimatedSection className="px-6 py-24">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mt-12 grid md:grid-cols-4 gap-6 md:gap-4 relative">
            <div className="hidden md:block absolute left-0 right-0 top-5 h-px bg-gradient-to-r from-[#6366F1] to-[#8B5CF6]" />
            {STEPS.map((step, i) => (
              <div key={step.title} className="relative md:text-center">
                ...
                <p className="mt-2 text-sm text-[#A0A0B8]">{step.body}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="mt-12 grid md:grid-cols-3 gap-5">
            {QUOTES.map((q) => (
              <motion.article
                whileHover={reduceMotion ? {} : { scale: 1.02 }}
                key={q.author}
                className="rounded-2xl bg-[#0F0F1A] border border-[#1E1E35] p-6 hover:shadow-[0_0_36px_rgba(139,92,246,0.2)] transition-all"
              >
                <p className="text-[#FFFFFF] leading-relaxed">"{q.text}"</p>
                <p className="mt-5 text-sm text-[#A0A0B8]">— {q.author}</p>
              </motion.article>
            ))}
          </div>
          <div className="mt-10 flex flex-wrap justify-center gap-3">
            {["ESCA", "UM6P", "UIR"].map((logo) => (
              <span key={logo} className="px-4 py-2 rounded-full border border-[#2A2A45] bg-[#0E0E18] text-[#8E8EA8] text-sm">
                {logo}
              </span>
            ))}
          </div>
      ...
      <AnimatedSection className="px-6 py-24">
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-10 items-center">
          <div>
            <p className="text-[#C8102E] tracking-[0.15em] text-xs font-semibold">FOR INSTITUTIONS</p>
            <h2 className="mt-3 text-3xl sm:text-4xl font-semibold">Give your students an unfair advantage</h2>
            <p className="mt-5 text-[#A0A0B8] leading-relaxed">
            ...
            <Link
              to="/auth"
              className="inline-flex mt-7 rounded-xl px-6 py-3 text-sm font-medium bg-[#C8102E] hover:bg-[#DA1B3A] transition-colors"
            >
              Request a demo →
            </Link>
            <ul className="mt-7 space-y-3 text-[#D8D8EA]">
              {[
          ...
          <motion.div
            whileHover={reduceMotion ? {} : { scale: 1.02 }}
            className="rounded-2xl border border-[#2A2A46] p-6 bg-gradient-to-br from-[#111124] to-[#0B0B16] shadow-[0_0_40px_rgba(99,102,241,0.2)]"
          >
      ...
      <AnimatedSection className="px-6 py-24">
        <div className="max-w-5xl mx-auto rounded-3xl border border-[#2A2A45] bg-gradient-to-r from-[#111126] to-[#0A0A14] p-10 text-center">
          <h2 className="text-3xl sm:text-4xl font-semibold">Your career starts with clarity.</h2>
          <p className="mt-4 text-[#A0A0B8]">Join thousands of Moroccan students building their future with AI.</p>
          <Link
            to="/auth"
          ...
          <p className="mt-4 text-sm text-[#7A7A94]">Free via your university · No credit card required</p>
        </div>
      </AnimatedSection>

      <footer className="px-6 py-10 border-t border-[#1B1B32] bg-[#050508]">
        <div className="max-w-6xl mx-auto border-t border-transparent pt-2 [border-image:linear-gradient(to_right,transparent,#3A3A64,transparent)_1]">
          <div className="grid md:grid-cols-3 gap-6 items-center text-sm">
            <div>
              <p className="font-semibold text-white">Cariva</p>
              <p className="text-[#A0A0B8] mt-1">AI-native career intelligence</p>
            </div>
            <div className="flex flex-wrap gap-4 md:justify-center text-[#B6B6CF]">
              {["Product", "For Universities", "About", "Contact"].map((l) => (
                <span key={l}>{l}</span>
              ))}
            </div>
            <div className="md:text-right">
              <span className="inline-flex text-xs rounded-full border border-[#2A2A45] bg-[#0E0E18] px-3 py-1 text-[#C7C7E4]">
                Powered by Claude AI
              </span>
              <p className="text-[#8A8AA5] mt-2">© 2026 Cariva. All rights reserved.</p>
            </div>
          </div>
        </div>
      </footer>
